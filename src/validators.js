...
const sizeValidator = (req, res, next) => {
	if (req.method === 'POST' || req.method === 'PUT') {
		if (Object.keys(req.body).length > 0) {
			...
			if (memorySize > config.JSON_MAX_SIZE) {
				throwError('JSON body is too large. Should be less than '+config.JSON_MAX_SIZE+'KB', 413);
			} else if (Array.isArray(req.body)) {
				if (req.body.length > config.ARRAY_MAX_LENGTH) {
					throwError('Not more than '+config.ARRAY_MAX_LENGTH+' records for bulk upload.', 413);
				} else next();
			} else next();
...
const validateParams = (req, res, next) => {
	...
	} else if (req.box.length < config.BOXID_MIN_LENGTH || req.box.length > 64) {
		throwError('Box id must be atleast '+config.BOXID_MIN_LENGTH+' chars long & max. 64 chars.');
...
const authenticateRequest = async (req, res, next) => {
	try {
		if (req.method === 'GET' || req.method === 'POST'){
			if ([config.API_KEY_LIMITED,config.API_KEY_ALL].includes(req['apiKey'])) next();
					else throwError('Invalid API_KEY.', 401);
		}else if(req.method === 'PUT' || req.method === 'DELETE') {
			if (config.API_KEY_ALL == req['apiKey']) next();
					else throwError('Invalid API_KEY for delete.', 401);
		} else next();
	} catch (error) {
		next(error);
	}
